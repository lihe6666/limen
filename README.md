...
- **二级 LLM 研判**(异步 + 缓存,**多 provider 可插拔**):规则判为"可疑"的灰色请求送 LLM 定夺。
  - 支持 `anthropic`(Claude)、`openai_compat`(OpenAI / Ollama / vLLM / DeepSeek / Groq 等所有 OpenAI 兼容端点)、`gemini`。
  - 换厂商只改配置;统一结构化 JSON 裁决;TTL 缓存避免重复调用;超时/故障按 `fail_open`/`fail_close` 降级,绝不阻塞主链路。
...
[llm]
enabled = true
provider = "openai_compat"              # anthropic | openai_compat | gemini
model = "gpt-4o-mini"
base_url = "http://localhost:11434/v1"  # openai_compat 端点;Ollama 示例。留空则用各 provider 默认
api_key_env = "OPENAI_API_KEY"          # 从该环境变量读取密钥(绝不写死);openai_compat 可留空
...
- `openai_compat` → `https://api.openai.com/v1`(`Authorization: Bearer`)
