...
cargo run --release -- eval [样本目录]  # 离线评测规则引擎,默认 testdata/blazehttp
```
...
`testdata/blazehttp/` 是 BlazeHTTP 测试集（658 黑样本 + 33,219 白样本，原始 HTTP 请求格式），**已 gitignore、GPL-3.0 许可，不得提交进仓库**；缺失时从 https://github.com/chaitin/blazehttp 获取 testcases。
